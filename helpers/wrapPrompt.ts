...
function getTimezone(timezone: string | null) {
  const tz = TIMEZONES.find((t) => t.abbr === (timezone || 'EST'));
...
export function wrapPrompt(prompt: string | null, timezone: string | null) {
  ...
  const metaprompt = `You are an AI agent talking to a human over the phone. Today is ${today}. Your replies are short (1-2 sentences). Follow the prompt below as closely as possible:`;
